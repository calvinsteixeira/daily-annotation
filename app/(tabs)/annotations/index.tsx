import { FlatList, View, Alert, Modal } from 'react-native'
import { Text, Skeleton, Button, useTheme } from '@rneui/themed'
import React from 'react'
import { MainContainer, PreviewAnnotation, SelectBox, CustomModal } from '@/components'
...
export default function Index() {
  ...
  return (
    <MainContainer>
      <CustomModal title={'Novo processo'} visible={visibleModal} onRequestClose={() => setVisibleModal(false)}>
        <Text> Algum conteúdo aqui</Text>
      </CustomModal>
      <View style={{ flex: 1, paddingTop: 30 }}>
        <Text style={globalTextStyles.titlePage}>Meus processos</Text>
        <Text style={{ marginTop: 6 }}>Aqui você encontra todas as suas anotações, filtre pelo ano e mês se preferir.</Text>
        <View style={{ flexDirection: 'row', gap: 10 }}>
          <View style={{ flex: 1, marginTop: 26, flexDirection: 'column', gap: 6 }}>
        ...
        <Text style={{ marginTop: 20, marginBottom: 12 }}>
          Confira suas anotações do mês de <Text style={{ fontWeight: 700 }}>{monthFilterValue.label}</Text>:
        </Text>
        {loadingData ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <PreviewAnnotation id={item.id} createdAt={item.createdAt} description={item.description} humorLevel={item.humorLevel} resume={item.resume} onDelete={handleDeleteAnnotation} />
            )}
          />
        )}
      </View>
